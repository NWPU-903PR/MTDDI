...
class Optimizer(object):
    def __init__(self, embeddings, latent_inters, latent_varies,
        ...
        self.inputs_neg = placeholders['batch_neg']
        ...
        self.batch_node = placeholders['batch_node']
        self.adj_min_batch = placeholders['adj_min_batch']
        self.sim_min_batch = placeholders['sim_min_batch']
        self.row_inputs = tf.squeeze(gather_cols(self.inputs, [0]))
        self.col_inputs = tf.squeeze(gather_cols(self.inputs, [1]))

        self.row_inputs_neg = tf.squeeze(gather_cols(self.inputs_neg, [0]))
        self.col_inputs_neg = tf.squeeze(gather_cols(self.inputs_neg, [1]))
        ...
        # self.neg_preds = self.batch_predict(self.row_inputs_neg, self.col_inputs_neg)
    ...
    def _build(self):
        # self.cost = self._hinge_loss(self.outputs, self.neg_outputs) + self._sim_loss(self.adj_min_batch,self.batch_node,self.embeddings)
        self.cost = FLAGS.gamma * self._xent_loss(self.outputs, self.neg_outputs)  + FLAGS.alpha * self._sim_loss(self.adj_min_batch,self.sim_min_batch,self.batch_node,self.embeddings)
    ...
    def _xent_loss(self, aff, neg_aff):
        ...
        return loss
    def _sim_loss(self,adj_min_batch,sim_min_batch,batch_node,embeddings):
        batch_embedding = tf.gather(embeddings[0],batch_node)
        # D = tf.diag(tf.reduce_sum(adj_min_batch,1))
        # L = D - adj_min_batch
        D = tf.diag(tf.reduce_sum(sim_min_batch, 1))
        L = D - sim_min_batch
        result = 2*tf.trace(tf.matmul(tf.matmul(tf.transpose(batch_embedding),L),batch_embedding))
        return result
